  ...
  return <section className="py-16 bg-secondary/30">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-3 md:grid-cols-3 gap-8 items-center justify-items-center">
          {logos.map((logo, index) => <div key={index} className="flex items-center justify-center p-6 opacity-60 hover:opacity-100 transition-opacity duration-300">
              <img src={logo.src} alt={logo.alt} className="h-36 w-auto object-contain filter grayscale hover:grayscale-0 transition-all duration-300" />
            </div>)}
        </div>
      </div>
    </section>;
